...
function activateVoice(alpha, beta, gamma){
    ...
    if(alpha > 120){
        let myTxt = "Why the fuck would you rotate me?";  
...
function activateVoice2(alpha, beta, gamma){
	if(beta > 90){
		let life = "What the fuck are you doing with your life?";
		sayItLoud(life);
		
		document.getElementById('voice').innerHTML = life;
	}
	else{
		document.getElementById('voice').innerHTML = "";
	}
...
function showMyImage(alpha, beta, gamma){
    ...
    if(beta > 40){
        document.getElementById('myImage').src = "Animepolpo.png"; // show image
        }
        else {
...
function showMyImage2(alpha, beta, gamma){
    // image if Y-axis > value
    if(alpha > 0){
